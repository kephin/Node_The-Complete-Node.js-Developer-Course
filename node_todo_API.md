...
Connect to the local server and create a model with certain attributes, so mongoose knows how to store data
...
Use **Postman**, which is a central tool for building a REST APIs, helps you create HTTP request and fire them off.
...
**[Review]** Getting an individual resourse - GET /todos/:id
...
**[Update]** Update a resource - PATCH /todos/:id
...
Add `npm start` for Heroku and specify node version
...
Set up database with Heroku add-on
...
Setting up a test database for testing, which we should have done at development
